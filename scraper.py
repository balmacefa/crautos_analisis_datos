...
async def ensure_url_list_exists(page):
    ...
    try:
        ...
        logging.error("No se encontró el botón de 'Última Página'. Puede que solo haya una página de resultados.")
        # If the button doesn't exist, assume there's only one page.
        last_page_number = 1
    except Exception as e:
        logging.error(f"Error al buscar la última página: {e}. Abortando recolección de URLs.")
        return
            ...
            for link in links:
                ...
                if href:
                    ...
                    if absolute_url not in detail_urls:
                        ...
                        page_urls_found += 1
            logging.info(f"Se encontraron {page_urls_found} nuevas URLs. Total de URLs únicas: {len(detail_urls)}")
            
        except PlaywrightTimeoutError:
            logging.warning(f"No se encontraron links de autos en la página {page_num} o la página no cargó correctamente.")
